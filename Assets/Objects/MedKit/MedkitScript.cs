...
public class MedkitScript : MonoBehaviour
{
	...
	public void UsedInInventory()
	{
		// Almost the same as the Interact funtion, but the UnitScript is implyed and we dont have to destroy an object (it doesn't exist)
		UnitScript who = GameObject.FindWithTag("Player").GetComponent<UnitScript>();
		who.Conditions.Add(new Medicine(displayImage, conditionColor, Mathf.CeilToInt(HealAmount / 4f * 3f), HealSpeed));
		who.HP += Mathf.FloorToInt(HealAmount / 4f);
	}

	public void PickUp(InventoryScript inventory)
	{
		if (CanInteract) {
			CanInteract = false;
			inventory.AddAnItem(InventoryObject);
			Destroy(gameObject);
		}
	}
}
